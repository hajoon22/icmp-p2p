...
uint16_t send_lookup_request(struct traversal_session *ts, uint8_t *pub, uint8_t *priv, uint32_t daddr, uint16_t dport, uint8_t f) {
	...
	size_t n = build_lookup_request(&buf, nonce, pub, priv, ts->mapped_port, want, f);
	if (n < 1) return 0;

	// lookup request to bootstrap
	for (int i = 0; i < MAX_RETRY; i++) {
		if (traversal_send(ts, daddr, dport, buf, n) < 0) {
			continue;
		}
...
static void send_lookup_response(struct traversal_session *ts, uint8_t *pub, uint8_t *priv, uint32_t addr, uint16_t port, uint8_t want, uint16_t nonce, uint8_t f) {
	uint8_t *buf = NULL;
	size_t n = build_lookup_response(addr, &buf, pub, priv, want, nonce, f);
	if (n < 1) return;

	for (int i = 0; i < MAX_RETRY; i++) {
		if (traversal_send(ts, addr, port, buf, n) < 0) {
			continue;
		}
...
void parse_lookup_request(struct traversal_session *ts, uint8_t *pub, uint8_t *priv, struct icmp_unreach *rp) {
	...
	send_lookup_response(ts, pub, priv, ntohl(rp->iph.saddr), port, want, nonce, free_slots());
}
