...
static int add_peer(uint8_t *pub, uint32_t addr, uint16_t port, uint8_t fs, uint32_t source, uint16_t nonce, bool bootstrap) {
    struct in_addr a = {
        .s_addr = htonl(addr)
    };
...
int new_peer(uint8_t *pub, uint32_t addr, uint16_t port, uint8_t fs, uint32_t source, uint16_t save_nonce, uint16_t check_nonce, bool bootstrap) {
    for (int i = 0; i < next_index; i++) {
        ...
        if (p->address == addr) {
            if (!pub) return 0;
            if (check_nonce != 0 && p->nonce != check_nonce) return -1; // invalid nonce
    ...
    return add_peer(pub, addr, port, fs, source, save_nonce, bootstrap);
}
